...
const joinRoom = function(this: Socket<ServerToClientEvents>, data: chatJoinAttributes) {
    const { room, token }: chatJoinAttributes = data
    
    if(!room || !token)  this.emit("chat:demit", {status:"failure", message:"Room or Token not found!"})

    try {
        jwt.verify(token, process.env.REFRESH_TOKEN_SECRET)
    } catch (_error: unknown) {
        this.emit("chat:demit", { status: "failure", message: "Unauthorized! Authentication required" })
    }

    this.join(room)
}

const sendMessages = async function (this: Socket<ServerToClientEvents>, data: chatMessageAttributes) {
    ...
    if (receiverID === undefined) return this.to(room).emit("chat:receive", {status:"successfully", message })
    if(!message) return this.emit("chat:receive", {status:"failure", message: "Message cennot be empty!"})
