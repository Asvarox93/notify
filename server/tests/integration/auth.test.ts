  ...
  describe("/api/auth/refresh route", () => {
    ...
    describe("given invalid refresh token", () => {
      ...
      it("should return 401 status code if no ID in refresh token", async () => {
        const refToken = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJmaXJzdE5hbWUiOiJTZWJhc3RpYW4iLCJsYXN0TmFtZSI6IkJpYWxlayIsIm5pY2tuYW1lIjoiQXN2YXJveCIsImlhdCI6MTY1ODM4NDY0MCwiZXhwIjoxNjU4OTEwMjQwfQ.6T9V1lBjqiuHsRlQp4jfFnJoOoneJzRJ_Mb0VGbx49U";
        const response = await request(app)
          .post("/api/auth/refresh")
          .set("Content-type", "application/json")
          .send({
